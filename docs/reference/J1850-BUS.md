...
| 6 | Grey | +12V Ignition (switched) | Power | **Blue** | **cluster power source.** Full path (split point S2): pin 6 → **F1 2A inline in the harness** → signal board terminal (1,3) → row-1 rail (Q2's emitter taps off here) → transit (18,2) → power board → D2 → TVS1 → Mini560 → D4 → board 5V header. The signal board is upstream of D2, so it is fused but **not** reverse-protected. See [`../../firmware/docs/reference/bike-power-injection.md`](../../firmware/docs/reference/bike-power-injection.md) and [`../schematics/bike-power-chain.py`](../schematics/bike-power-chain.py). **⚠ Pin-6 conflict, OPEN:** this same pin is also listed as the sixth divider input (lane G6 "ignition"). Since pin 6 powers the board, G6 can only ever read "on". Options are laid out in [`../phases/phase3-cluster.md`](../phases/phase3-cluster.md); not yet decided. |
...
ultrasonic fuel-sender caveat are in the Phase 3 section of
...
> cold-start), stock cluster still attached, **no DTCs** (ROADMAP Phase 2
> Stage 4 + [`../../firmware/docs/rides/stage4-tx-bench-log.md`](../../firmware/docs/rides/stage4-tx-bench-log.md)).
...
`firmware/main/engine/j1850/j1850_vpw.c` (host-tested, round-tripped).

## J1850 VPW message decode table

Calibrated against real on-bike captures. The decoder is
`firmware/main/engine/j1850/j1850_parse.c` (host-tested); the DTC codec is `dtc.c`.
...
[`../../firmware/docs/reference/j1850-undecoded-frames.md`](../../firmware/docs/reference/j1850-undecoded-frames.md).

### Diagnostic trouble codes (DTCs)
The HD J1850 read/clear protocol (request/response bytes, per-module addresses)
and the on-bike validation are in
[`../../firmware/docs/reference/dtc-read-probe.md`](../../firmware/docs/reference/dtc-read-probe.md);
the codec is `firmware/main/engine/j1850/dtc.c` (host-tested).
