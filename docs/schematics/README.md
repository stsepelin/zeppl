...
| `j1850_signal_board.py/.svg` | Phase 3 + 6 — full bike signal board | Physical pad-per-hole layout of the whole signal board: the J1850 transceiver (as `j1850_perfboard`) **plus the 6 discrete 12V dividers** (`discrete_divider`, ×6: turn L/R, beam, neutral, oil, ignition). Bike build (no Rpd). Superset of `j1850_perfboard`. The power chain lives on a **separate** board (switcher noise / current). Electrical source of truth stays `j1850_rx`/`j1850_tx`/`discrete_divider`. **matplotlib, not schemdraw.** |
