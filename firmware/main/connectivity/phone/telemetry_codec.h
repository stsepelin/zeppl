...
//             u8  clock_hours
//             u8  clock_minutes
//             u8  status             cluster state bits, see TELEMETRY_STATUS_*
//
...
#define TELEMETRY_PAYLOAD_LEN 34u
#define TELEMETRY_FRAME_LEN   (3u + TELEMETRY_PAYLOAD_LEN)  // 37

// status byte: lets the companion reflect the cluster's real layout state (so
// its Layout toggle isn't a lie) and know whether the map is usable.
#define TELEMETRY_STATUS_MAP_SUPPORTED (1u << 0)  // a map driver is compiled in
#define TELEMETRY_STATUS_LAYOUT_MAP    (1u << 1)  // current layout is the map view
#define TELEMETRY_STATUS_MAP_AVAILABLE (1u << 2)  // the map screen loaded OK
...
// Encode `vd` + the cluster `status` bits into `out`. Returns the number of
// bytes written (TELEMETRY_FRAME_LEN) or 0 if the buffer is too small.
